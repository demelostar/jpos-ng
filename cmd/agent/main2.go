...
func main() {
	...
	var serverAddr = flag.String("binding", "", "binding to proxy (domain:port)")
	...
	if *bindAddr != "" {
		...
		for {
			...
			logrus.Infof("Got bindingion from: %s\n", conn.RemoteAddr())
			tlsConn := tls.Server(conn, &tlsConfig)

			if err := binding(tlsConn); err != nil {
				logrus.Error(err)
			}
		}
	}
	...
	if *serverAddr == "" {
		logrus.Fatal("please, specify the target host user -binding host:port")
	}
	host, _, err := net.SplitHostPort(*serverAddr)
	if err != nil {
		logrus.Fatal("invalid binding address, please use host:port")
	}
	...
	for {
		...
		if *socksProxy != "" {
			...
			conn, err = sDial(*serverAddr, *socksProxy, *socksUser, *socksPass)
		} else {
...
func sDial(serverAddr string, socksProxy string, socksUser string, socksPass string) (net.Conn, error) {
...
func binding(conn net.Conn) error {
